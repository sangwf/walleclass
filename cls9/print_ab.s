...
;系统调用0x84，打印EDX为首地址的字符串
mov eax, 0x00080000 ;段选择符0x08
lea edx, [int_print_string]
mov ax, dx
...
;set ds, 这里如果不设置，后续每次用户态内存操作，都得初始化ds
mov eax, 0x17
mov ds, ax
...
int_fork: ;fork系统调用处理函数
    ...
    mov eax, [SS: ESP + 20] ;ESP
                            ;[esp + 16] = eflags
                            ;[esp + 20] = esp
    mov [task1_esp], eax

    mov [task1_ebp], ebp ;EBP

    ;复制用户栈的内容，函数调用（如call fork_syscall）会有栈操作
    push es
    mov ax, ds
    mov es, ax

    lea si, [usr_stk0_start]
    lea di, [usr_stk1_start]

    push ecx
    mov ecx, 512 ;用户栈是128个dd
    rep movsb
    pop ecx
...
task1_eax: ;用于设置eax时好找偏移
    dd 0, 0, 0, 0
task1_esp: ;用于设置esp时好找偏移
    dd usr_stk1 ;第14个dd是用户栈
task1_ebp: ;用于设置ebp时好找偏移
    dd 0, 0, 0 ; 第15个dd是ebp
